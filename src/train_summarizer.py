...
def prepare_data(pre, save_path='../data/post_pre.csv'):
    ...
    summary = ['[START] '+ str(doc) + ' [END]' for doc in nlp.pipe(processed_summary, batch_size=5000)]
    print('Added [START] and  [END] labels to articles')
...
def train_model(post_pre, model_path, test_size=0.1, max_text_len=1700, max_summary_len=150):
   ...
   encoder_model.save(model_path + '/encoder.h5')
   decoder_model.save(model_path + '/decoder.h5')
...
if __name__ == '__main__':
    ...
    #post_pre = pd.read_csv('../data/post_pre.csv').sample(500)
    ...
    encoder_model = tf.keras.models.load_model(model_path + '/encoder.h5')
    decoder_model = tf.keras.models.load_model(model_path + '/decoder.h5')

    x_tr_seq = x_tokenizer.texts_to_sequences(post_pre.text)
    y_tr_seq = x_tokenizer.texts_to_sequences(post_pre.summary)
    x_tr = pad_sequences(x_tr_seq,  padding='post', maxlen=max_text_len)
    y_tr = pad_sequences(y_tr_seq,  padding='post', maxlen=max_summary_len)
